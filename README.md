...
```text
-Docker Desktop (macOS/Windows) 또는 Docker Engine (Linux) - docker compose (Docker v2 이상) 추가(권장): - jq: Prometheus API 결과를 보기 좋게 출력
...
## 3. 설정 파일 요약
```text
각 파일은 “로컬에서 재현 가능한 운영형 알림 파이프라인” 구현을 위해 역할이 분리되어 있습니다.
```
...
# 7. 트러블슈팅 체크리스트
```text
운영에서 빈도가 높은 문제를 기준으로 “증상 → 확인 → 조치” 형태로 정리
```
